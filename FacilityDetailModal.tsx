...
export default function FacilityDetailModal({
  ...
  const kakaoMapUrl = `https://map.kakao.com/link/to/${encodeURIComponent(facility.name)},${facility.lat},${facility.lng}`;
  const naverMapUrl = `https://map.naver.com/v5/search/${encodeURIComponent(facility.name)}?c=${facility.lng},${facility.lat},15,0,0,0,dh`;
          ...
          <div className="flex items-start gap-3.5">
            ...
            <div>
              <div className="flex items-center gap-1.5 flex-wrap">
                {facility.district && (
                  <span className="text-xs font-bold px-2.5 py-0.5 rounded-full bg-slate-800 text-white shadow-xs">
                    {facility.district}
                  </span>
                )}
                ...
                {facility.capacity && (
                  <span className="text-xs font-bold px-2.5 py-0.5 rounded-full border bg-emerald-50 text-emerald-800 border-emerald-300">
                    🚲 {facility.capacity}대 보관
                  </span>
                )}
                {facility.airPumpType && (
                  <span className="text-xs font-bold px-2.5 py-0.5 rounded-full border bg-sky-50 text-sky-800 border-sky-300">
                    💨 {facility.airPumpType}
                  </span>
                )}
        ...
        <div className="mt-5 space-y-2">
          <div className="grid grid-cols-2 gap-2">
            <a
              ...
              className="flex items-center justify-center gap-1.5 rounded-xl bg-[#FEE500] hover:bg-[#FADA0A] text-[#191919] font-extrabold text-xs py-3 px-2 shadow-xs transition-transform active:scale-95"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-1.5 rounded-xl bg-[#03C75A] hover:bg-[#02B351] text-white font-extrabold text-xs py-3 px-2 shadow-xs transition-transform active:scale-95"
            >
              <span className="text-xs">🟢</span>
              <span>네이버지도 길찾기</span>
            </a>
          ...
          <button
            type="button"
            onClick={onClose}
            className="w-full rounded-xl bg-slate-100 border border-slate-200 py-3 text-xs font-bold text-slate-700 hover:bg-slate-200"
          >
            닫기
          </button>
        </div>
