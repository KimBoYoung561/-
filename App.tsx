...
import HomeExploreSheet from './components/HomeExploreSheet';
    ...
    >
        ...
        {!isExpanded && (
          <div className="w-full mt-1 flex items-center justify-center">
            <span className="text-[10px] font-bold text-slate-400 flex items-center gap-1 bg-slate-100 px-3 py-0.5 rounded-full">
              <ChevronUp size={11} className="text-[#0055FF] animate-bounce" /> 위로 올려서 기상청 안전 수칙 & 도로 정보 보기
            </span>
          </div>
        )}
      </div>

      {/* Primary Action Buttons & Real-time Weather Safety Tip (Always Visible) */}
      <div className="px-5 pb-3 pt-1 space-y-2">
        {/* Quick Weather Safety Alert Strip */}
        <WeatherCyclingSafetyBanner weather={weather} compact={true} />
      ...
      {isExpanded && (
        <div className="px-5 pb-5 pt-2 max-h-[58vh] overflow-y-auto hide-scrollbar space-y-3.5 border-t border-slate-200/80 mt-1 animate-in fade-in slide-in-from-bottom-3 duration-200">
          ...
          {/* 1. Real-time KMA Cycling Safety & Bridge Gust Banner (기상청 초단기실황 연동 안전 라이딩 수칙) */}
          <WeatherCyclingSafetyBanner weather={weather} compact={false} showChecklist={true} />

          {/* 2. In-Sheet Filter Category Chips (경치 좋은, 평지 중심 등 키워드) */}
...
export default function App() {
  ...
  // Anyang Tourism & Attractions state
  const [selectedTourSpot, setSelectedTourSpot] = useState<AnyangTourSpot | null>(null);
  const [tourCategoryFilter, setTourCategoryFilter] = useState<string>('all');
  const [mapCenter, setMapCenter] = useState<{ lat: number; lng: number }>(ANYANG_CENTER);
  ...
  const [gpsMessage, setGpsMessage] = useState<string>('GPS 신호 탐색 중...');
  const [isGpsModalOpen, setIsGpsModalOpen] = useState(false);
  const [isLocatingUser, setIsLocatingUser] = useState(false);
  const [isMapPickLocationMode, setIsMapPickLocationMode] = useState(false);
  ...
  // GPS Auto-location on startup & Continuous Live Time & Real-time GPS Tracking
  useEffect(() => {
    const updateTimes = () => {
    ...
    const initGps = async () => {
      ...
      setGpsMessage('위성 GPS 및 네트워크 위치 탐색 중...');

      try {
        const pos = await getRobustCurrentPosition((_stage, msg) => {
          if (isMounted) setGpsMessage(msg);
        });
        ...
        setGpsMessage(
          pos.isHighAccuracy
            ? `정밀 위성 GPS 수신 (오차 ±${Math.round(pos.accuracy || 10)}m)`
            : `Wi-Fi/네트워크 보정 위치 수신`
        );
        ...
        try {
          ...
          // fallback handled by riderPosition effect
        }
      } catch (err: any) {
        if (!isMounted) return;
        console.warn('Initial GPS acquisition failed:', err);
        const isDenied = err?.message === 'PERMISSION_DENIED';
        const isTimeout = err?.message === 'TIMEOUT';
        setGpsStatus(isDenied ? 'denied' : isTimeout ? 'timeout' : 'error');
        setIsGpsActive(false);
        setGpsMessage(
          isDenied
            ? '브라우저 위치 권한이 차단되어 있습니다.'
            : 'GPS 신호가 약하여 안양 중심 기본값으로 시작합니다.'
        );

        // Fallback if browser didn't provide location
        setRiderPosition({
          lat: ANYANG_CENTER.lat,
          lng: ANYANG_CENTER.lng,
        });
        const fallbackAddr = await coordToAddress(ANYANG_CENTER.lat, ANYANG_CENTER.lng);
        if (isMounted) {
          setOrigin(fallbackAddr || '경기도 안양시 만안구 안양동 (안양천 쌍개울)');
        }
      }

      // Continuous resilient watch
      stopWatch = startRobustGpsWatch(
        (pos) => {
          if (!isMounted) return;
          setRiderPosition({ lat: pos.lat, lng: pos.lng });
          setGpsAccuracy(pos.accuracy || null);
          setGpsStatus('active');
          setIsGpsActive(true);
          if (pos.heading !== null && pos.heading !== undefined && !isNaN(pos.heading)) {
  ...
  // Synchronize readable address & weather whenever riderPosition updates
  ...
  useEffect(() => {
    ...
    // 1. Reverse geocode to Korean road/dong address if moved > 50m
    const shouldGeocode =
      !lastGeocodedCoordRef.current ||
      Math.abs(lastGeocodedCoordRef.current.lat - riderPosition.lat) > 0.0005 ||
      Math.abs(lastGeocodedCoordRef.current.lng - riderPosition.lng) > 0.0005 ||
      origin === '위치 확인 중...';

    if (shouldGeocode) {
      lastGeocodedCoordRef.current = { lat: riderPosition.lat, lng: riderPosition.lng };
      coordToAddress(riderPosition.lat, riderPosition.lng)
        .then((addr) => {
          if (isMounted && addr && addr !== '내 현재 위치') {
            setOrigin(addr);
          }
        })
        .catch(() => {
  ...
  /* Find My Location (GPS) with 2-Stage Robust Fallback */
  const handleFindMyLocation = async (showModalOnFailure = true) => {
    setIsLocatingUser(true);
    setGpsStatus('searching');
    setGpsMessage('위치 탐색 진행 중...');
    setOrigin('GPS 위치 확인 중...');
    ...
    try {
      ...
      const isDenied = err?.message === 'PERMISSION_DENIED';
      const isTimeout = err?.message === 'TIMEOUT';
      const status: GpsStatus = isDenied ? 'denied' : isTimeout ? 'timeout' : 'error';
      setGpsStatus(status);
      setIsGpsActive(false);
      ...
      if (isDenied) {
        ...
        setGpsMessage('GPS 수신 시간 초과 (음영지역)');
      } else {
      ...
      if (!riderPosition) {
        ...
        setOrigin('경기도 안양시 만안구 안양동 (안양천 쌍개울)');
      }
    } finally {
  ...
  /* Set Location to Preset */
  const handleSelectGpsPreset = (preset: GpsHubPreset) => {
    setRiderPosition({ lat: preset.lat, lng: preset.lng });
    setMapCenter({ lat: preset.lat, lng: preset.lng });
    setOrigin(preset.address);
    setGpsStatus('active');
    setIsGpsActive(true);
    setGpsAccuracy(10);
    setGpsMessage(`선택된 거점: ${preset.name}`);
  };

  /* Set Manual Location via Map Click */
  const handleSetManualRiderPosition = async (lat: number, lng: number) => {
    setRiderPosition({ lat, lng });
    setMapCenter({ lat, lng });
    setIsMapPickLocationMode(false);
    setGpsStatus('active');
    setIsGpsActive(true);
    setGpsAccuracy(10);
    setGpsMessage('지도에서 직접 지정한 위치');

    try {
      const address = await coordToAddress(lat, lng);
      setOrigin(address || '지정된 위치');
    } catch {
      setOrigin('지정된 위치');
    }
  };
  ...
  const handleFindOptimalRoute = async (params: RouteSearchParams) => {
    ...
    const destCoords = resolvedDestCoords || ANYANG_CENTER;
    ...
    <div className={`app-scale relative h-[100dvh] w-full overflow-hidden bg-slate-100 select-none ${fontScaleClass}`}>
      ...
      <div className="relative h-full w-full max-w-md mx-auto bg-slate-50 overflow-hidden flex flex-col shadow-2xl border-x border-slate-200">
        ...
        <div className={`relative flex-1 w-full ${currentTab === 'home' ? 'block' : 'hidden'}`}>
          <MapComponent
            center={mapCenter || riderPosition || ANYANG_CENTER}
            ...
            tourSpots={ANYANG_TOUR_DATA.filter((s) => tourCategoryFilter === 'all' || s.category === tourCategoryFilter)}
            highlightTourSpotId={selectedTourSpot?.id || null}
            onSelectTourSpot={(spot) => {
              setSelectedTourSpot(spot);
              setMapCenter({ lat: spot.lat, lng: spot.lng });
            }}
            reports={reports}
            onSelectReport={handleSelectReport}
            onMapClick={(lat, lng) => {
              if (isMapPickLocationMode) {
                handleSetManualRiderPosition(lat, lng);
                return;
              }
            ...
            onFindMyLocation={() => handleFindMyLocation(true)}
            isRiding={appState === 'riding'}
            isSheetExpanded={appState === 'riding' ? isRidingSheetExpanded : false}
          />

          {/* Map Pick Location Mode Top Floating Indicator */}
          {isMapPickLocationMode && (
            <div className="absolute top-16 left-3 right-3 z-40 animate-in fade-in slide-in-from-top-2">
              <div className="flex items-center justify-between p-3 rounded-2xl bg-slate-900/95 text-white shadow-2xl border border-white/20 backdrop-blur-md">
                <div className="flex items-center gap-2 text-xs font-bold">
                  <MapPin size={16} className="text-[#0055FF] animate-bounce shrink-0" />
                  <span>지도에서 내 위치로 지정할 곳을 탭하세요</span>
                </div>
                <button
                  type="button"
                  onClick={() => setIsMapPickLocationMode(false)}
                  className="px-2.5 py-1 rounded-lg bg-white/20 hover:bg-white/30 text-[11px] font-bold transition-colors"
                >
                  취소
                </button>
              </div>
            ...
            <div className="absolute left-0 right-0 top-0 z-30 px-3 pt-3 pointer-events-none">
              <div className="pointer-events-auto flex items-center justify-between gap-2 rounded-2xl bg-white/95 p-2.5 shadow-xl backdrop-blur-xl border border-slate-200">
                <button
                  ...
                  className="flex min-w-0 items-center gap-2.5 text-left flex-1 group hover:opacity-90 active:scale-[0.99] transition-all"
                  title="내 현재 GPS 위치로 지도 이동 및 주소 새로고침"
                >
                  <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-white shadow-xs group-hover:scale-105 transition-transform ${
                    gpsStatus === 'active'
                      ? 'bg-[#0055FF]'
                      : gpsStatus === 'denied'
                      ? 'bg-rose-500'
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <span className="text-[10px] font-black text-[#0055FF]">내 현재 위치</span>
                      {gpsStatus === 'active' ? (
                        <span
                          onClick={(e) => {
                            e.stopPropagation();
                          ...
                          className="inline-flex items-center gap-1 rounded-full bg-emerald-50 border border-emerald-200 px-1.5 py-0.2 text-[9px] font-bold text-emerald-700 hover:bg-emerald-100 transition-colors cursor-pointer"
                          title="GPS 상태 및 오차 확인 / 설정 열기"
                        >
                          <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse" />
                          <span>실시간 GPS {gpsAccuracy ? `(±${Math.round(gpsAccuracy)}m)` : ''}</span>
                        </span>
                      ) : gpsStatus === 'searching' || isLocatingUser ? (
                        <span
                          onClick={(e) => {
                            e.stopPropagation();
                            setIsGpsModalOpen(true);
                          }}
                          className="inline-flex items-center gap-1 rounded-full bg-blue-50 border border-blue-200 px-1.5 py-0.2 text-[9px] font-bold text-blue-700 animate-pulse cursor-pointer"
                        >
                        ...
                        <span
                          ...
                          className="inline-flex items-center gap-1 rounded-full bg-rose-50 border border-rose-200 px-1.5 py-0.2 text-[9px] font-bold text-rose-700 hover:bg-rose-100 transition-colors cursor-pointer"
                          title="위치 권한 차단됨 - 클릭하여 해결"
                        >
                          <ShieldAlert size={9} />
                          <span>권한 차단됨 (해결)</span>
                        </span>
                        ...
                        <span
                          ...
                          className="inline-flex items-center gap-1 rounded-full bg-amber-50 border border-amber-200 px-1.5 py-0.2 text-[9px] font-bold text-amber-700 hover:bg-amber-100 transition-colors cursor-pointer"
                          title="GPS 신호 지연 - 클릭하여 해결"
                        >
                    ...
                    <div className="truncate text-xs font-black text-slate-900 mt-0.5 tracking-tight" title={origin}>
                      {origin}
                    </div>
                  </div>
                </button>
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  <button
                    ...
                    className="flex h-9 w-9 items-center justify-center rounded-xl border border-slate-200 bg-slate-100 text-slate-700 hover:bg-blue-50 hover:text-[#0055FF] active:scale-95 transition-all"
                    title="GPS 위치 수신 및 권한 관리 창 열기"
                    aria-label="GPS 설정"
                  >
                    <Radio size={15} className={gpsStatus === 'active' ? 'text-[#0055FF]' : 'text-slate-600'} />
                  </button>
                  <button
                    type="button"
                    onClick={() => handleFindMyLocation(true)}
                    disabled={isLocatingUser}
                    className="flex h-9 w-9 items-center justify-center rounded-xl border border-slate-200 bg-slate-100 text-slate-700 hover:bg-slate-200 active:scale-95 transition-all disabled:opacity-50"
                    title="현재 위치 즉시 재탐색"
                  >
                    <Compass size={16} className={`text-[#0055FF] ${isLocatingUser ? 'animate-spin' : ''}`} />
                  </button>
                  <button
                    type="button"
                    onClick={() => setIsQuickReportOpen(true)}
                    className="flex h-9 items-center justify-center rounded-xl border border-red-200 bg-red-50 px-2.5 text-red-600 hover:bg-red-100 active:scale-95 transition-all"
                    title="장애물 신고"
                  >
                  ...
                  {isBottomSheetOpen ? (
                    ...
                    <>
                      ...
                      <HomeExploreSheet
                        ...
                        onSelectCategory={setTourCategoryFilter}
                        onSelectSpot={(spot) => {
                          setSelectedTourSpot(spot);
                          setMapCenter({ lat: spot.lat, lng: spot.lng });
                        }}
                    ...
                    <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      <div className="flex flex-col min-w-0">
                        ...
                        <span className="text-[11px] text-slate-500 truncate">
                          {weather?.summary ? weather.summary : '안양 9경 및 편의시설 둘러보기 (탭하여 펼치기)'}
                        </span>
                      </div>
                    </div>
                    <div className="flex items-center gap-1 shrink-0 text-[#0055FF] pr-1">
                      <span className="text-xs font-bold">펼치기</span>
                      <ChevronUp size={16} />
                    </div>
                  </div>
                ) : (
        ...
        <AttractionDetailModal
          spot={selectedTourSpot}
          isOpen={!!selectedTourSpot}
          onClose={() => setSelectedTourSpot(null)}
          weather={weather}
          onFocusOnMap={(spot) => {
            setMapCenter({ lat: spot.lat, lng: spot.lng });
          }}
        ...
        {/* GPS Troubleshooting and Signal Management Modal */}
        <GpsTroubleshootModal
          ...
          onRetryGps={() => handleFindMyLocation(false)}
          onSelectPreset={handleSelectGpsPreset}
          onEnableMapPickMode={() => {
            setIsMapPickLocationMode(true);
